...
class MyCard(QWidget):
    ...
    def __init__(self, parent: QWidget = None, title_text: str = None, objectName: str = None,
                 size: tuple = (200, 300), margin: tuple = (8, 8, 8, 8), can_collapse: bool = True) -> None: 
        super().__init__(parent)
        self.font_manager = ModQtFont()

        # 状态：normal 或 collapsed
        self.status = "normal"

        self.collapse_enabled = can_collapse
        self.parent = parent

        # 保存原始尺寸和折叠后的尺寸
        self._original_size = size
        self._collapsed_size = (size[0], 30)  # 折叠后高度为30，宽度不变
        ...
        if self.collapse_enabled:
            self.btn_collapse_toggle = MyRoundButton(self, svg_path="Images/BtnCardToggleNormal.svg", size=(32, 32))
            # 将按钮放在右上角
            self.btn_collapse_toggle.move(size[0] - margin[2] - 20, margin[1])
            # 连接按钮点击事件
            self.btn_collapse_toggle.clicked.connect(self.toggle_collapse)
    ...
    def collapse(self):
        """折叠卡片"""
        if self.status == "collapsed" or not self.collapse_enabled:
            return
            
        # 设置动画
        self.animation.setStartValue(self.size())
        self.animation.setEndValue(QRect(0, 0, *self._collapsed_size).size())
        
        # 更新按钮文本
        self.btn_collapse_toggle.setSvgPath("Images/BtnCardToggleCollapsed.svg")
        ...
        for child in self.children():
            if child != self.title_label and child != self.btn_collapse_toggle:
                child.hide()
    
    def uncollapse(self):
        """展开卡片"""
        if self.status == "normal" or not self.collapse_enabled:
            return
        ...
        # 更新按钮文本
        self.btn_collapse_toggle.setText("↑")
    ...
    def resizeEvent(self, event):
        ...
        if hasattr(self, 'btn_collapse_toggle'):
            # 更新按钮位置到右上角
            self.btn_collapse_toggle.move(self.width() - self.btn_collapse_toggle.width() - 8, 8)
